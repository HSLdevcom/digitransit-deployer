...
const NOW = new time.Date().getTime();
...
describe('cron-service-restarter', function() {
  ...
  it('no apps should restart when last restart within default limit interval', () => {
    ...
    const restartAtString = restartAt.getHours() + ":" + restartAt.getMinutes();
    const testApps = [
      appConfig('/app1', new time.Date(NOW - minutes(15 * 60)),{"restart-at": restartAtString}, true)
    ];
    restarter.command(testApps, failIfRestart);
  });
  ...
  it('no apps should restart when last restart within user set limit interval', () => {
    ...
    const restartAtString = restartAt.getHours() + ":" + restartAt.getMinutes();
    const testApps = [
      appConfig('/app1', new time.Date(NOW - minutes(110)),{"restart-at": restartAtString, "restart-limit-interval": "120"}, true)
    ];
    restarter.command(testApps, failIfRestart);
  });

  it('no apps should restart when it has been over hour since when the service was supposed to restart', () => {
    const restartAt = new time.Date(NOW - minutes(350));
    restartAt.setTimezone('Europe/Helsinki');
    const restartAtString = restartAt.getHours() + ":" + restartAt.getMinutes();
    const testApps = [
      appConfig('/app1', new time.Date(NOW - minutes(600)),{"restart-at": restartAtString, "restart-limit-interval": "120"}, true)
    ];
    restarter.command(testApps, failIfRestart);
  });

  it('no apps should restart when service is not stable', () => {
    const restartAt = new time.Date(NOW - minutes(42));
    restartAt.setTimezone('Europe/Helsinki');
    const restartAtString = restartAt.getHours() + ":" + restartAt.getMinutes();
    const testApps = [
      appConfig('/app1', new time.Date(NOW - minutes(150)),{"restart-at": restartAtString, "restart-limit-interval": "120"}, false)
    ];
    restarter.command(testApps, failIfRestart);
  });

  it('stable app with last restart outside of the user set limit interval should restart', () => {
    const restartAt = new time.Date(NOW - minutes(1));
    restartAt.setTimezone('Europe/Helsinki');
    const restartAtString = restartAt.getHours() + ":" + restartAt.getMinutes();
    const testApps = [
      appConfig('/app1', new time.Date(NOW - minutes(150)),{"restart-at": restartAtString, "restart-limit-interval": "120"}, false),
      appConfig('/app2', new time.Date(NOW - minutes(150)),{"restart-at": restartAtString, "restart-limit-interval": "120"}, true),
      appConfig('/app3', new time.Date(NOW - minutes(100)),{"restart-at": restartAtString, "restart-limit-interval": "120"}, true)
    ];
  ...
  it('stable app with restart-at less than 60 mins before should restart', () => {
    const restartAt = new time.Date(NOW - minutes(55));
    restartAt.setTimezone('Europe/Helsinki');
    const restartAtString = restartAt.getHours() + ":" + restartAt.getMinutes();
    const testApps = [
      appConfig('/app1', new time.Date(NOW - minutes(23 * 60)),{"restart-at": restartAtString}, true)
  ...
  it('stable app with one restart-at time less than 60 mins before should restart once', () => {
    const restartAtFirst = new time.Date(NOW - minutes(55));
    const restartAtSecond = new time.Date(NOW - minutes(350));
  ...
  it('stable app with two restart-at time less than 60 mins before should restart once', () => {
    const restartAtFirst = new time.Date(NOW - minutes(25));
    const restartAtSecond = new time.Date(NOW - minutes(55));
    restartAtFirst.setTimezone('Europe/Helsinki');
    restartAtSecond.setTimezone('Europe/Helsinki');
    const restartAtString = restartAtFirst.getHours() + ":" + restartAtFirst.getMinutes() + ", "
      + restartAtSecond.getHours() + ":" + restartAtSecond.getMinutes();
